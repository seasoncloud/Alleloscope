...
#' @param rhohats: n by m matrix of rho_hat values for each cell across the regions. 
#' @param thetahats: n by m matrix of theta_hat values for each cell across the regions. 
#' @param snpCoverages: n by m matrix of total read counts covering SNPs for each cell across the regions.
...
#' 
#' @export
AssignClones_ref<-function(rhohats, thetahats, snpCoverages, priorCloneProbs=NULL, clone.genotypes, sigma.rho){
  ...
  return(list(probClone=probClone, cloneAssign=cloneAssign, cloneConf=cloneConf))
}