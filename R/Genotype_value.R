...
#' @param cov_only Logical (TRUE/FALSE). Whether or not to use only compute fold chagne values for each cell across the regions.
#' 
#' @return (rho_hat, theta_hat) of each cell for all region in the "genotype_values".
#' Every 2 columns in the genotype_values are (rho_hat, theta_hat) of each region. Each row is a cell.
#'
#' @export
Genotype_value=function(Obj_filtered=NULL, type="tumor", raw_counts=NULL, ref_counts=NULL, cov_adj=1, ref_gtv=NULL, mincell=NULL, qt_filter=TRUE,  cell_filter=TRUE, refr=TRUE, cov_only=FALSE){
